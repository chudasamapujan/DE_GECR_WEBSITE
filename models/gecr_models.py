...
class Student(db.Model):
    ...
    def enrollment_no(self):
        """Alias for roll_no field"""
        return self.roll_no
    
    @property
    def batch(self):
        """Calculate batch based on current year and semester"""
        current_year = datetime.now().year
        # Assuming 4-year course, calculate admission year
        years_completed = (self.semester - 1) // 2 if self.semester else 0
        admission_year = current_year - years_completed
